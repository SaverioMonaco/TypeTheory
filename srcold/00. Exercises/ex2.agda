...
map f [] = [] -- if empty list, do nothing
map f (x ∷ xs) = f x ∷ map f xs  -- apply f to the prepending element and
                                 -- map to the rest of the list

-- Function that add one (succ zero) to any natural number,
-- we'll map this function to a list
add1 : ℕ → ℕ
add1 n = n + succ zero
