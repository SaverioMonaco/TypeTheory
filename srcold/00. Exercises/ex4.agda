...
-- this just tells agda which operation should be computed
-- first, if you use parenthesis you should not worry about it
infix  3 _∎
infixr 2 _≡⟨_⟩_ _≡⟨⟩_
infix  1 begin_
...
lemma-succ-pred f = g zero (f zero)
  where
  g : (a : ℕ) → succ (pred a) ≡ a → ⊥
  g zero ()
  g (succ a) _ = g a (f a)
...
-- this is a fix of the previous lemma which led to a contradiction
-- now that we restrict to positive number it is true that
-- for every number succ (pred a) is a
-- Goal: (a : ℕ) → Positive a → succ (pred a) ≡ a
-- ————————————————————————————————————————————————————————————
...
-- Goal: succ (x + succ y) ≡ succ (succ (x + y))
--       ———————————————————————————————————————
--       y : ℕ, x : ℕ
lemma-+-succ (succ x) y = cong succ (lemma-+-succ x y)
...
module _ {A : Set} where
  ...
  -- Goal: (ys : List A) (x : A) → reverse (ys ∷ʳ x) ≡ (x ∷ reverse ys)
  --       ————————————————————————————————————————————————————————————
  --       A : Set
  ...
  --       ————————————————————————————————————————————————————
  --       x,y  : A, ys : List A, A : Set
  -- (Explicit) lemma-reverse-∷ʳ (y ∷ ys) x = reverse ((y ∷ ys) ∷ʳ x)  
  -- (Direct computation using properties) reverse ((y ∷ ys) ∷ʳ x) = reverse (ys ∷ʳ x) ∷ʳ y
  -- (just use cong for λ _ ∷ʳ y and this lemma recursively)
